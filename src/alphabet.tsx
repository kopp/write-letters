...
export function pathFromLetter(letter: Letter, x: number, y: number, scale: number, reference: "anchor" | "first"): Path2D {
    const transformPoint: (point: ControlPoint) => [number, number] = (() => {
        ...
        if (reference === "first") {
            ...
            return point => [x + (point.x - firstPoint.x) * scale, y - (point.y - firstPoint.y) * scale];
        }
